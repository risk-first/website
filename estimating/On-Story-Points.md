...
(Sometimes, developers provide _tolerances_ around their story-point estimates, "oh, that'll take 2-4 days".  Usually, this subtlety gets lost in the planning process.  We'll come back to this). 
...
Can we bootstrap the estimation process by providing an "Interference Checklist" for story-points, based on the things that commonly throw spanners into the works?  Below, I've sketched out a small section of what this might look like.  The [next article](Interference-Checklist.md) contains a more complete Interference Checklist that I use and you can modify for your own purposes.
...
|                                              |                                                                                   |           |                 |
| **\- [Implementation Risk](../risks/Feature-Risk.md#implementation-risk)** | Developer unfamiliar with the requirements / system |           |                 |                                              | **\- [Feature-Fit](Feature-Risk.md#feature-fit-risk)**| Success criteria hard to define                                          |           |                 |
...
The more options we have over the size of a story, the more difficult it is to be right.  Also fewer possible sizes means we get more experience estimating work to fit that particular size.   Here, we'll stick with "Small", "Medium" and "Large", and we'll set bounds according to the time they'll likely take, and, crucially, a risk budget:
...
You don't have to use exactly these sizes.  Use whatever works for your team, but keep the number of sizes _low_ and the maximum length _short_: anything longer than this becomes unwieldy, and lacks a rapid feedback loop.  Anything shorter doesn't really need an estimate anyway.
...
So this piece of work _exceeds_ the risk budget for a "Small" item, and needs to be classed as at least "Medium".  Note that this is just a small sample of the full Interference Checklist, the real risk scores are likely to go a lot higher... your mileage may vary.
